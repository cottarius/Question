...
\t1. Статический полиморфизм (compile-time polymorphism): Основывается на методах перегрузки (method overloading). Java выбирает, какой из перегруженных методов вызывать, во время компиляции, основываясь на сигнатуре метода (количестве и типе параметров).\n
                                   \t2. Динамический полиморфизм (runtime polymorphism): Основывается на методах переопределения (method overriding). Java определяет, какой метод вызывать, во время выполнения программы. Обычно это достигается через виртуальные методы и наследование.', 'CORE1', true),
